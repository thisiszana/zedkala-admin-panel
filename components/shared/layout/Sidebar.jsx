...
export default function Sidebar() {
  ...
  return (
    ...
    >
      <div
        className={`flex items-center justify-between fixed bg-white dark:bg-slate-700 p-4 top-0 right-0 z-20 border-l border-gray-200 transition-all duration-300 ${
          isCollapsed ? "min-w-[80px]" : "min-w-[250px]"
        }`}
      >
      ...
      <nav className="pt-[74px] pb-5 mt-[60px] transition-opacity duration-300 ease-in-out">
        ...
        <div className="mr-4 mb-2 mt-5">
          <h1 className="text-p1 text-gray-400">نمای کلی</h1>
        </div>
            ...
            <Fragment key={item.title}>
              ...
              {item.title === "داشبورد" && !isCollapsed && (
                <div className="ml-4 mb-2 mt-5">
                  <h1 className="text-p1 text-gray-400">مدیریت</h1>
                </div>
              )}
              {item.title === "ایجاد وبلاگ" && !isCollapsed && (
                <div className="ml-4 mb-2 mt-5">
                  <h1 className="text-p1 text-gray-400">تنظیمات</h1>
                </div>
              )}
            </Fragment>
          ))}
